...
try:
    from src.params import TRAFFIC_NUM_VEHICLES, TRAFFIC_MAX_TIME, TRAFFIC_HOTSPOT_RATIO
except Exception:
...
def generate_traffic(
    ...
    origins = [
        ...
        if e.getFunction() != "internal" and not e.getIncoming()
    ]
    destinations = [
        e.getID()
        for e in net.getEdges()
        if e.getFunction() != "internal" and not e.getOutgoing()
    ]
